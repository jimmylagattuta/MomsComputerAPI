# app/services/ringcentral/reject_call_party.rb

require "ringcentral"

module Ringcentral
  class RejectCallParty
    def self.call(event)
      new(event).call
    end
    ...
    def call
      ...
      path = "/restapi/v1.0/account/~/telephony/sessions/#{event.telephony_session_id}/parties/#{event.party_id}/reject"

      response = rc.post(path, payload: {})

      Rails.logger.info(
        "[RingCentral Reject] Success " \
    ...
    def failure!(reason)
      Rails.logger.error(
        "[RingCentral Reject] Cannot reject call " \
